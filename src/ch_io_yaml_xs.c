...
#include "ch_io_yaml.h"

int
ch_io_yaml_xs_coords (yaml_parser_t *parser, ChXSCoords *xs_coords)
{
  assert (parser);
  assert (xs_coords);
...
int
ch_io_yaml_xs (yaml_parser_t *parser, ChXSDefinition *xs_def)
{
      ...
      switch (type)
        ...
        case YAML_SCALAR_EVENT:
          ...
          if (strncmp ("coordinates", scalar_value, CH_MAX_SCALAR_CHAR) == 0)
            {
              if (ch_io_yaml_xs_coords (parser, &(xs_def->coordinates)))
                goto error;
            }
